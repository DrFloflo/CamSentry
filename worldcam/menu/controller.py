...
def consume_menu_changes(menu_state: MenuState, selected_class_names: set[str]) -> tuple[bool, bool, bool, bool]:
...
def snapshot_menu_state(menu_state: MenuState, selected_class_names: set[str]) -> tuple[set[str], bool, bool, float]:
    """Return a stable snapshot of selected classes, feature toggles, and display threshold."""
    return set(selected_class_names), menu_state.pose_enabled, menu_state.sahi_enabled, menu_state.display_threshold
...
def handle_class_menu_key(
    ...
    """Update menu state and return class-change, pose-toggle, SAHI-toggle, and threshold-change flags."""
    if key == ord("m"):
